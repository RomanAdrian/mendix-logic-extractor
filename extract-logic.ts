...
import { type IModel, projects, domainmodels, microflows, security } from "mendixmodelsdk";
    ...
    for (const docInterface of module.documents) {
        ...
        if (docInterface.structureTypeName === "Microflows$Microflow") {
            ...
            for (const obj of microflow.objectCollection.objects) {
                ...
                if (obj instanceof microflows.ActionActivity) {
                    ...
                    if (action) {
                        ...
                        if (action instanceof microflows.RetrieveAction) {
                            ...
                            if (source instanceof microflows.AssociationRetrieveSource) activityData.details.source = source.associationQualifiedName;
                            if (source instanceof microflows.DatabaseRetrieveSource) activityData.details.entity = source.entityQualifiedName;
                ...
                if (activityData) activities.push(activityData);
            }
            microflowsData.push({
                name: microflow.name,
